...
export class SubscriptionService {
   ...
   */
  async purchase(userId: string, plan: SubscriptionPlan, months: number, amount: Prisma.Decimal | string | number, currency: Currency) {
  ...
  /* --------------------- приватные утилиты --------------------------------- */
  private async runWithPurchaseLock<T>(userId: string, plan: SubscriptionPlan, fn: () => Promise<T>): Promise<T | undefined> {
  ...
  private async scheduleExpireJob(userId: string, endsAt: Date) {
    ...
    await this.queueService.scheduleJob(QUEUE_SUBSCRIPTIONS, QueueSubscriptionJob.Expire, { userId }, endsAt, jobId);
  }
  ...
  private async deleteNotifyBeforeExpireJob(userId: string) {
    await this.redisService.deleteByPattern(`${BULL_KEY.USER_SUBSCRIPTION_NOTIFY_EXPIRE}:${userId}:*`);
  }
}
