...
export class SentryService {
  ...
  startSpanManual(opts: { name: string; op?: string; forceTransaction?: boolean }): Span {
    return this.sentry.startSpanManual({ name: opts.name, op: opts.op, forceTransaction: opts.forceTransaction }, span => span);
  }

  async withSpan<T>(opts: { name: string; op?: string }, cb: (span: Span) => Promise<T>): Promise<T> {
