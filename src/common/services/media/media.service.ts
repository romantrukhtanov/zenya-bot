...
export class MediaService extends MediaUtils {
	...
	async sendText(
		ctx: Context | number,
		text: string,
		extra: ExtraOptions & ExtraReplyMessage = {},
	): Promise<number> {
	...
	async editText(
		...
		messageId: number,
		text: string,
		extra: Exclude<ExtraOptions, 'keyboard'> = {},
	): Promise<void> {
		const chatId = this.getChatId(ctx);
	...
	private async uploadVideoAndSave(
		ctx: Context | number,
		filePath: string,
		fileName: string,
		extra: ExtraOptions = {},
		...
		if (await fileExists(localPath)) {
			const msg = await this.replyWithVideo(
				ctx,
	...
	private async uploadPhotoAndSave(
		ctx: Context | number,
		filePath: string,
		fileName: string,
		extra: ExtraOptions = {},
		...
		if (await fileExists(localPath)) {
			const msg = await this.replyWithPhoto(
				ctx,
				{ source: createReadStream(localPath), filename: fileName },
				extra,
			);
	...
	private async uploadFileAndSave(
		ctx: Context | number,
		filePath: string,
		fileName: string,
		extra: ExtraOptions = {},
		...
		if (await fileExists(localPath)) {
			const msg = await this.replyWithDocument(
				ctx,
				{ source: createReadStream(localPath), filename: fileName },
				extra,
			);
