...
export class AddMetaCardWizard extends BaseWizardScene<AddMetaCardWizardContext> {
  ...
  async getContent(@Ctx() ctx: AddMetaCardWizardContext) {
    ...
    if (questions.length === 0) {
      await ctx.reply('Не удалось распознать вопросы. Пожалуйста, введите вопросы, разделяя их символом новой строки:');
      return;
    }
  ...
  async finishStep(@Ctx() ctx: AddMetaCardWizardContext) {
    const buttons = [Markup.button.callback(TEXTS.ADD_NEW_META_CARD, BaseCallback.Restart), ...this.sceneButtons];
