...
export class AddCategoryWizard extends BaseWizardScene<AddCategoryWizardContext> {
  ...
  async start(@Ctx() ctx: AddCategoryWizardContext) {
    try {
      ...
      const buttons = chapters.map(chapter => Markup.button.callback(chapter.name, `${SceneCommand.SELECT_CHAPTER}:${chapter.id}`));
  ...
  async onSelectChapter(@Ctx() ctx: AddCategoryWizardContext) {
    ...
    const chapterName = ctx.wizard.state.chapters?.find(chapter => chapter.id === chapterId)?.name || 'Unknown';
  ...
  async createCategory(@Ctx() ctx: AddCategoryWizardContext) {
    try {
      ...
      const keyboard = Markup.inlineKeyboard(
        [Markup.button.callback(TEXTS.CREATE_NEW_CATEGORY, BaseCallback.Restart), ...this.sceneButtons],
        { columns: 1 },
      );
