...
export class AddFactsWizard extends BaseWizardScene<AddFactsWizardContext> {
	...
	async start(@Ctx() ctx: AddFactsWizardContext) {
		...
		const buttons = chapters.map((chapter) =>
			Markup.button.callback(chapter.name, `select_chapter_${chapter.id}`),
		);
	...
	async getFacts(@Ctx() ctx: AddFactsWizardContext) {
		...
		if (facts.length === 0) {
			await ctx.reply(
				'Не удалось распознать факты. Пожалуйста, введите факты, разделяя их символом новой строки:',
			);
			return;
		}
	...
	async finishStep(@Ctx() ctx: AddFactsWizardContext) {
		const buttons = [
			Markup.button.callback(TEXTS.CREATE_NEW_FACTS, BaseCallback.Restart),
			...this.sceneButtons,
		];
