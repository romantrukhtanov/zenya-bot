...
export class OnboardingWizard extends BaseWizardScene<OnboardingWizardContext> {
	...
	async onChapterSelect(@Ctx() ctx: OnboardingWizardContext) {
		...
		await this.sendOrEdit(
			ctx,
			translations.scenes.onboarding.chooseCategory,
			getCategoriesButtons(categories),
			'MarkdownV2',
		);
	}
	...
	async onCategorySelect(@Ctx() ctx: OnboardingWizardContext) {
		...
		const interestingFactsButton = Markup.button.callback(
			`📚 ${translations.shared.interestingFacts}`,
			OnboardingCallback.SelectFacts,
		);
	...
	async onPracticeSelect(@Ctx() ctx: OnboardingWizardContext) {
		...
		const nextButton = Markup.button.callback(
			translations.shared.next,
			OnboardingCallback.PracticeFactNext,
		);
	...
	async getCongratsContent(@Ctx() ctx: OnboardingWizardContext) {
		...
		const nextButton = Markup.button.callback(
			translations.shared.next,
			OnboardingCallback.MetaCardsNext,
		);

		await this.ensureSceneMedia(ctx, OnboardingMedia.Congrats);
		await this.sendOrEdit(
			ctx,
			translations.scenes.onboarding.practiceFactCongrats,
			[nextButton],
			'MarkdownV2',
			true,
		);
	...
	async onMetaCardsNext(@Ctx() ctx: OnboardingWizardContext) {
		...
		const chatNextButton = Markup.button.callback(
			translations.shared.next,
			OnboardingCallback.ChatNext,
		);

		await this.sendOrEdit(
			ctx,
			translations.scenes.onboarding.chatInfo,
			[chatNextButton],
			'MarkdownV2',
		);
	}
	...
	async onChatNext(@Ctx() ctx: OnboardingWizardContext) {
		...
		const goToMainButton = Markup.button.callback(
			translations.scenes.onboarding.goToMain,
			BaseCallback.GoToMain,
		);

		await this.ensureSceneMedia(ctx, OnboardingMedia.Gift);
		await this.sendOrEdit(
			ctx,
			translations.scenes.onboarding.finish,
			[this.subscriptionButton, goToMainButton],
			'MarkdownV2',
		);
	}
