...
import {
	...
	SubscriptionStarsPrice,
	SubscriptionUzsPrice,
...
export class SubscriptionWizard extends BaseWizardScene<SubscriptionWizardContext> {
	...
	async createInvoice(@Ctx() ctx: SubscriptionWizardContext) {
		...
		const payload = await this.subscriptionSceneUtils.issue(
			userId,
			telegramUser.id,
			ctx.wizard.state.selectedPlan!,
			currency,
		);
	...
	get publicOfferButton() {
		return Markup.button.callback(
			translations.shared.publicOffer,
			SubscriptionCallback.PublicOffer,
		);
	}
	...
	async onPaymentSuccess(@Ctx() ctx: SubscriptionWizardContext) {
		...
		const amount = isCurrencyUsdUzs(currency)
			? SubscriptionUzsPrice[plan]
			: SubscriptionStarsPrice[plan];

		const subscriptionId = await this.subscriptionService.purchaseByTelegramId(
			telegramUserId,
			plan,
			1,
			amount,
